 ...
 * Driven by useMusicPlayerStore (chat bubbles enqueue tracks here).
 */
export function GlobalMusicBar() {
  ...
  useEffect(() => {
    ...
    fetch(streamSrc, { headers: { Range: 'bytes=0-1' }, credentials: 'same-origin' })
      .then((r) => setPreview(r.headers.get('X-Aurora-Preview') === '1'))
  ...
  useEffect(() => {
    const a = audioRef.current
    if (!a) return
    const onTime = () => setProgress(a.currentTime, Number.isFinite(a.duration) && a.duration > 0 ? a.duration : duration)
  ...
  }, [duration, playNext, setPlaying, setProgress])
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-[calc(3.5rem+env(safe-area-inset-bottom))] z-[60] flex justify-center px-3 md:bottom-4">
      <div
        className={cn(
          'pointer-events-auto flex w-full max-w-lg items-center gap-3 rounded-2xl border border-border/60 bg-background/95 p-2.5 shadow-xl backdrop-blur-xl',
        )}
      >
        <button
          type="button"
