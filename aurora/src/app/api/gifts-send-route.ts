...
import { GIFT_COIN_REASONS, serializeGiftMetadata } from '@/lib/gifts'
...
export const POST = withJsonApi(async function POST(req: NextRequest) {
  ...
  try {
    const giftSent = await db.giftSent.create({
      data: {
        giftId: gift.id,
        senderId: me.id,
        recipientId,
    ...
    // Send push notification to offline chat members
    ...
    // Refund coins if gift delivery fails after deduction
